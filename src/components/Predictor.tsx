...
export function Predictor({ dataMap, cityName }: PredictorProps) {
  ...
  return (
    <div className="card predictor-card" style={{ padding: '2rem' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column' }}>
                ...
                {fetchError && (
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', marginTop: '6px' }}>
                    <span style={{ fontSize: '0.75rem', color: palette.summer }}>{t('predictor.fetchError')}</span>
                    <button 
                      onClick={loadEnso}
                      style={{ 
      ...
      {!result ? (
        <div style={{ textAlign: 'center', color: palette.muted, padding: '2rem' }}>{t('predictor.invalidData')}</div>
      ) : (
        <div className="predictor-results" style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
          ...
          <div className="predictor-metric-card predictor-metric-card--temperature" style={{ background: '#fffbeb', border: '1px solid #fde68a', padding: '1.5rem', borderRadius: '12px', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            <h3 className="predictor-metric-title" style={{ margin: 0, color: palette.wetBulb, fontSize: '1.1rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
              <ThermometerSun size={18} strokeWidth={2} aria-hidden="true" />
              {t('predictor.tempTitle')}
            </h3>
            <div>
              <div style={{ fontSize: '0.9rem', color: palette.wetBulb, marginBottom: '0.2rem' }}>{t('predictor.daytimeHigh')}</div>
              <div style={{ fontSize: '1.8rem', fontWeight: 700, color: palette.autumn }}>
                {result.tMaxRange[0]}<span style={{ fontSize: '1.2rem', color: palette.wetBulb, margin: '0 4px' }}>~</span>{result.tMaxRange[1]}<span style={{ fontSize: '1.2rem' }}>℃</span>
              </div>
            </div>
            <div className="metric-row" style={{ display: 'flex', justifyContent: 'space-between' }}>
              <div>
                <div style={{ fontSize: '0.8rem', color: palette.wetBulb, marginBottom: '0.2rem' }}>{t('predictor.nighttimeLow')}</div>
                <div style={{ fontSize: '1.1rem', fontWeight: 600, color: palette.autumn, opacity: 0.8 }}>
                  {result.tMinRange[0]} ~ {result.tMinRange[1]} ℃
                </div>
              </div>
              <div>
                <div style={{ fontSize: '0.8rem', color: palette.wetBulb, marginBottom: '0.2rem' }}>{t('predictor.apparentTemp')}</div>
                <div style={{ fontSize: '1.1rem', fontWeight: 600, color: palette.autumn, opacity: 0.8 }}>
                  {result.atRange[0]} ~ {result.atRange[1]} ℃
                </div>
              </div>
              <div>
                <div style={{ fontSize: '0.8rem', color: palette.wetBulb, marginBottom: '0.2rem' }}>{t('predictor.dewPoint')}</div>
                <div style={{ fontSize: '1.1rem', fontWeight: 600, color: result.dewPointRange[1] >= 20 ? palette.heatWarning : palette.autumn, opacity: 0.8 }}>
          ...
          <div className="predictor-metric-card predictor-metric-card--rain" style={{ background: '#eff6ff', border: '1px solid #bfdbfe', padding: '1.5rem', borderRadius: '12px', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            <h3 className="predictor-metric-title" style={{ margin: 0, color: palette.rain, fontSize: '1.1rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
              <CloudRain size={18} strokeWidth={2} aria-hidden="true" />
              {t('predictor.rainTitle')}
            </h3>
            <div>
              <div style={{ fontSize: '0.9rem', color: palette.rainstorm, marginBottom: '0.2rem' }}>{t('predictor.rainProb')}</div>
              <div style={{ fontSize: '2rem', fontWeight: 700, color: palette.rain }}>
                {result.rainProb}%
              </div>
              <div style={{ fontSize: '0.85rem', color: palette.humidity, marginTop: '0.5rem' }}>
                {t('predictor.precipEstimate', { scale: t(`predictor.precipScale.${result.precipScale}`), value: result.precipExpected })}
              </div>
            </div>
            <div style={{ marginTop: 'auto' }}>
              <div style={{ fontSize: '0.8rem', color: palette.rainstorm, marginBottom: '0.2rem' }}>{t('predictor.humidityRange')}</div>
              <div style={{ fontSize: '1.1rem', fontWeight: 600, color: palette.rain, opacity: 0.8 }}>
          ...
          <div className="predictor-metric-card predictor-metric-card--risk" style={{ background: '#fef2f2', border: '1px solid #fecaca', padding: '1.5rem', borderRadius: '12px', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.8rem' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                <span style={{ fontWeight: 600, color: result.typhoonProb > 10 ? palette.summer : palette.heatWarning }}>{result.typhoonProb}%</span>
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <span style={{ color: palette.heatWarning, fontSize: '0.95rem' }}>{t('predictor.riskRain')}</span>
                <span style={{ fontWeight: 600, color: result.severeRainProb > 10 ? palette.summer : palette.heatWarning }}>{result.severeRainProb}%</span>
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <span style={{ color: palette.heatWarning, fontSize: '0.95rem' }}>{t('predictor.riskHeat')}</span>
                <span style={{ fontWeight: 600, color: result.heatProb > 10 ? palette.summer : palette.heatWarning }}>{result.heatProb}%</span>
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <span style={{ color: palette.heatWarning, fontSize: '0.95rem' }}>{t('predictor.riskCold')}</span>
                <span style={{ fontWeight: 600, color: result.coldProb > 10 ? palette.summer : palette.heatWarning }}>{result.coldProb}%</span>
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <span style={{ color: palette.heatWarning, fontSize: '0.95rem' }}>{t('predictor.heatStress')}</span>
                <span style={{ fontWeight: 600, color: result.heatStressProb > 20 ? palette.summer : palette.heatWarning }}>{result.heatStressProb}%</span>
              </div>
            </div>
            {result.heatStressProb > 0 && (
              <div style={{ fontSize: '0.8rem', color: palette.wetBulb, background: '#fef3c7', padding: '0.5rem', borderRadius: '4px' }}>
          ...
          <div className="predictor-metric-card predictor-metric-card--air" style={{ background: '#fdf4ff', border: '1px solid #f5d0fe', padding: '1.5rem', borderRadius: '12px', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            <h3 className="predictor-metric-title" style={{ margin: 0, color: palette.air, fontSize: '1.1rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
              <Wind size={18} strokeWidth={2} aria-hidden="true" />
              {t('predictor.airTitle')}
            </h3>
            <div>
              <div style={{ fontSize: '0.9rem', color: palette.air, marginBottom: '0.2rem' }}>{t('predictor.pm25Expected')}</div>
              <div style={{ fontSize: '2rem', fontWeight: 700, color: palette.air }}>
                {result.pm25Expected} <span style={{ fontSize: '1rem', fontWeight: 'normal' }}>μg/m³</span>
              </div>
              <div style={{ fontSize: '0.85rem', color: palette.air, marginTop: '0.5rem' }}>
                {t('predictor.airStandard')}
              </div>
            </div>
            <div style={{ marginTop: 'auto', display: 'flex', justifyContent: 'space-between' }}>
              <div>
                <div style={{ fontSize: '0.8rem', color: palette.air, marginBottom: '0.2rem' }}>{t('predictor.smogProb')}</div>
                <div style={{ fontSize: '1.1rem', fontWeight: 600, color: result.smogProb > 20 ? palette.airSevere : palette.air }}>
                  {result.smogProb}%
                </div>
              </div>
              <div>
                <div style={{ fontSize: '0.8rem', color: palette.air, marginBottom: '0.2rem' }}>{t('predictor.severeSmogProb')}</div>
                <div style={{ fontSize: '1.1rem', fontWeight: 600, color: result.severeSmogProb > 10 ? palette.airSevere : palette.air }}>
